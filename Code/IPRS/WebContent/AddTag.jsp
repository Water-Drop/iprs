...
<body>
	<div id="containdiv">
		<div id="tagdiv" style="float: left;">
			<%
				List<Paper> paperlist = (List<Paper>) request.getAttribute("paperlist");
				 out.print("<p>Select Paper to add tags:</p>");
			...
			<p>Tag1:</p>
			<p>
				<input type="text" id=tag1 size=25>
			</p>
			<p>Tag2:</p>
			<p>
				<input type="text" id=tag2 size=25>
			</p>
			<p>Tag3:</p>
			<p>
				<input type="text" id=tag3 size=25>
			</p>
			<input type="button" id="button1" value="submit" onclick="add_tags()" />
		</div>
		<div id="rdfdiv" style="text-align: center;">
			<%
			out.print("<br/>");
				out.print("<p>Select Paper to store(RDF):</p>");
				 out.print("<select id=\"paperselect\">");
				 if (paperlist != null) {
				     for(int i=0, j=paperlist.size(); i<j; i++) {
				     	Paper paper = (Paper) paperlist.get(i);
			...
			%>
			<br/>
			<input type="button" id="button2" value="Stored with RDF"
				onclick="store_RDF()" />
		</div>
